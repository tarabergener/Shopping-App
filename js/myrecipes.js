...
// Search JSON for myrecipes including ingredients specified by user
...
// populate cards onto the myrecipe page from JSON data
document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayRecipes(recipes) {
        ...
        recipes.forEach(recipe => { // for each recipe in myrecipes
    ...
    recipeContainer.addEventListener('submit', function(event) {
        event.preventDefault();
        // find recipe; convert to lowercase
        const query = recipeFinder.value.toLowerCase().trim();
        if (!query) return;
